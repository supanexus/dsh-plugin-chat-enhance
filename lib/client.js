window.__ModuleLoader__.load({
	...
	factory: (require) => {
			...
			ctx.inject([
				...
				scope.effect(() => {
					...
					if (!armVisionGate()) {
						...
						for (const ms of [0, 50, 200, 1000, 3000]) timers.push(setTimeout(() => {
							if (armVisionGate()) {
								for (const timer of timers) clearTimeout(timer);
							} else if (ms === 3000) console.error("[supanexus-chat-enhance] official conversation.composer.bar entry missing; skip image gate");
						}, ms));
					}
					return () => {
						for (const timer of timers) clearTimeout(timer);
		...
		*/
		...
		/**
		* Register searchable model picker UI.
