window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		//#endregion
		//#region lib/types/client/VisionGatedInputBar.js
		/**
		...
		*/
		function createVisionGatedInputBar(OfficialInputBar, capabilityCache, modelDirectories) {
			return function VisionGatedInputBar(props) {
				const sessionId = props.sessionId;
				const directoryCurrent = (0, react.useSyncExternalStore)((onStoreChange) => {
				...
				const current = directoryCurrent ?? modelSelection?.next ?? modelSelection?.lastUsed ?? null;
				...
				const supportsImage = current === null ? void 0 : capabilityCache.get(current.provider, current.model);
				const mediaTypes = (props.useProjection?.("imageLimits"))?.mediaTypes ?? FALLBACK_IMAGE_TYPES;
				const upstreamAddFiles = props.addFiles;
				const translate = props.t;
				const addFiles = (0, react.useCallback)((files) => {
					if (upstreamAddFiles === void 0) return null;
				...
				return (0, react.createElement)(OfficialInputBar, {
					...props,
					addFiles: upstreamAddFiles === void 0 ? void 0 : addFiles
				});
			};
		}
		//#endregion
		...
		*/
		function registerChatEnhance(ctx) {
			...
			ctx.inject([
				...
				let disposeGate;
				const armVisionGate = () => {
					if (disposeGate !== void 0) return true;
					const official = scope.slots.entries("conversation.composer.bar").find((entry) => entry.registrant !== REGISTRANT && entry.component !== void 0 && typeof entry.inject === "function");
					if (official === void 0 || official.inject === void 0) return false;
					...
					console.info("[supanexus-chat-enhance] image-aware composer.bar armed", { officialEntries: scope.slots.entries("conversation.composer.bar").length });
					return true;
				};
				scope.effect(() => {
					const timers = [];
					if (!armVisionGate()) {
						...
						for (const ms of [
							...
							if (armVisionGate()) for (const timer of timers) clearTimeout(timer);
							else if (ms === 3e3) console.error("[supanexus-chat-enhance] official conversation.composer.bar entry missing; skip image gate");
						}, ms));
					}
					return () => {
