 ...
 */
export function readRecentModels(storage = typeof localStorage === 'undefined' ? undefined : localStorage, key = RECENT_MODELS_STORAGE_KEY) {
    if (storage === undefined)
        return [];
    const current = storage.getItem(key);
    if (current !== null && current !== '')
        return parseRecentList(current);
 ...
 * @param key - storage key.
 */
export function pushRecentModel(entry, maxRecent, storage = typeof localStorage === 'undefined' ? undefined : localStorage, key = RECENT_MODELS_STORAGE_KEY) {
    if (storage === undefined)
        return [entry];
...
//# sourceMappingURL=recent-models.js.map